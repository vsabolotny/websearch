...
function toListing(item: Is24Item): Listing | null {
  if (!item.id) return null;
  const { priceText, perSqm, areaSqm } = readAttributes(item.attributes);
  // Commercial listings usually quote €/m²; derive an approximate monthly total.
...
/** Fetch commercial listings from ImmobilienScout24 for the configured region. */
